...
export default {
    ...
    methods: {
        ...
        oneStep() { //从第一步往下走
            ...
            //     this.city = sessionStorage.getItem('address').split(' ')
            //   }
            if (this.baseMessage.b_name && this.city.length && this.baseMessage.b_detail_address && this.baseMessage.b_uname && this.baseMessage.b_mobile && this.baseMessage.b_type !== '请选择商家类型') {
                if (this.checkbox1) {
                    this.axios.post('/wechatauth/apply/store/one', {
                        shopName: this.baseMessage.b_name,
                        shopUname: this.baseMessage.b_uname,
                        shopMobile: this.baseMessage.b_mobile,
                        province: this.city[0],
                        city: this.city[1],
                        county: this.city[2],
                        address: this.baseMessage.b_detail_address,
                        type: this.baseMessage.b_type == '个人' ? 0 : 1,
                    }).then(res => {
                        if (res.data.result) {
                            this.step = 2
                        } else {
        ...
        twoStep() { //第二步
            if (this.upImg.length > 0) {
                this.store_photo = this.upImg.join(',') // 数组转字符串
            }
            this.axios.post('/wechatauth/apply/store/two', {
                //   shopName: this.baseMessage.b_name,
                companyPhoto: this.photo,
                legalPersonPhotoFront: this.personFront,
                legalPersonPhotoRear: this.personBack,
                businessPhoto: this.licenseThumb,
                legalPersonBank: this.bank_thumb,
                companyStorePhoto: this.store_photo
            }).then(res => {
                if (res.data.result) {
