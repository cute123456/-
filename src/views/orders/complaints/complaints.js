...
export default {
    ...
    methods: {
        ...
        GoComment() { //去评论
            if (!this.comment) {
                this.$dialog.toast({ mes: '请输入售后描述', timeout: 2000 });
                return
            }
            if (!this.radio) {
                this.$dialog.toast({ mes: '请选择售后原因', timeout: 2000 });
                return
            }
            this.axios.post('/wechatauth/aftersale/add', {
                reason: this.radio,
                description: this.comment,
                orderId: this.$route.query.orderId
            }).then(res => {
                if (res.data.result) {
                    this.$dialog.toast({ mes: '提交成功,正在审核中', timeout: 3000 });
